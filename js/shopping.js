...
function createShoppingCard(item) {
  ...
  // 操作ボタン
  const actions = _createActionButtons(
    () => {
  ...
  actions.querySelector(".shopping-edit").addEventListener("click", () => {
  ...
  actions.querySelector(".shopping-delete").addEventListener("click", () => {
    db.collection("shopping").doc(item.id).delete();
    loadShoppingList();
  });
